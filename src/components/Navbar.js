...
// PERBAIKAN DI SINI: Menghapus ": any"
const iconMap = {
  Briefcase: <Briefcase />,
  FileText: <FileText />,
  HardHat: <HardHat />,
    ...
    <div className="hidden lg:block bg-[#0f172a] text-stone-300 text-[11px] font-medium py-2 relative z-[60]">
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
           <Clock size={12} className="text-[#3b82f6]" /> {/* Aksen icon top bar */}
    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 font-sans ${
      ...
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center relative">
        ...
        <Link href="/" className="text-2xl font-bold text-stone-900 tracking-tight z-50 flex items-center gap-1 group/logo">
           <div className="rounded-lg flex items-center justify-center text-white group-hover/logo:rotate-12 transition-transform">
             <Image
             src="/logometa.svg"
        ...
        <div className="hidden md:flex items-center space-x-1">
          <Link href="/" className="px-4 py-2 text-stone-600 font-medium hover:text-[#1e40af] text-sm transition rounded-full hover:bg-stone-50">Beranda</Link>
          ...
          >
            <button className={`flex items-center gap-1 px-4 py-2 text-sm font-medium transition rounded-full ${dropdownOpen ? 'text-[#1e40af] bg-stone-50' : 'text-stone-600 hover:text-[#1e40af] hover:bg-stone-50'}`}>
            ...
            <div className={`absolute top-full -left-48 w-[900px] bg-white rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-stone-100 p-2 transition-all duration-300 origin-top-left overflow-hidden ${dropdownOpen ? 'opacity-100 translate-y-2 visible' : 'opacity-0 translate-y-6 invisible'}`}>
               <div className="flex">
                        ...
                        <div className="space-y-1">
                          {items.map((item, idx) => (
                            <Link key={idx} href={`/layanan/${item.slug}`} className="flex items-center gap-3 p-2 rounded-xl hover:bg-stone-50 transition group/item">
                                <div className="w-8 h-8 rounded-lg bg-stone-100 text-stone-500 group-hover/item:bg-gradient-to-r group-hover/item:from-[#1e40af] group-hover/item:to-[#3b82f6] group-hover/item:text-white flex items-center justify-center transition-all duration-300">
                  ...
                  <div className="w-[250px] bg-stone-50 p-6 flex flex-col justify-between border-l border-stone-100">
                      ...
                      <Link href="https://wa.me/6281399710085" className="flex items-center justify-center gap-2 w-full py-3 bg-gradient-to-r from-[#1e40af] to-[#3b82f6] text-white rounded-xl text-sm font-bold hover:shadow-lg hover:shadow-blue-500/20 transition-all">
                          <Image src="/whatsapp-icon.svg" alt="WhatsApp" width={16} height={16} className="invert" /> Chat Admin
                      </Link>
                  </div>
               </div>
            </div>
          ...
          >
            <button className={`flex items-center gap-1 px-4 py-2 text-sm font-medium transition rounded-full ${toolsOpen ? 'text-[#1e40af] bg-stone-50' : 'text-stone-600 hover:text-[#1e40af] hover:bg-stone-50'}`}>
          ...
          <Link href="/blog" className="px-4 py-2 text-stone-600 font-medium hover:text-[#1e40af] text-sm transition rounded-full hover:bg-stone-50">Wawasan</Link>
          <Link href="/#tentang" className="px-4 py-2 text-stone-600 font-medium hover:text-[#1e40af] text-sm transition rounded-full hover:bg-stone-50">Tentang Kami</Link>
        ...
        <div className="hidden md:block ml-4">
          <Link href="https://wa.me/6281399710085" className="text-white px-5 py-2.5 rounded-full text-sm font-bold transition-all hover:shadow-lg hover:shadow-blue-500/20 flex items-center gap-2 hover:-translate-y-0.5 bg-gradient-to-r from-[#1e40af] to-[#3b82f6]">
      ...
      {isOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white shadow-2xl border-t border-stone-100 p-6 flex flex-col space-y-4 max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-3 pt-2 border-t border-stone-100">
             ...
             {Object.entries(groupedServices).map(([catName, items]) => (
                <div key={catName}>
                   ...
                   {items.map((item, idx) => (
                      <Link key={idx} href={`/layanan/${item.slug}`} onClick={() => setIsOpen(false)} className="block py-1.5 pl-2 text-sm font-medium text-stone-600 border-l-2 border-stone-100 hover:border-[#3b82f6] hover:text-[#1e40af]">
                        {item.title}
                      </Link>
                   ))}
                </div>
          ...
          <Link href="https://wa.me/6281399710085" className="w-full bg-gradient-to-r from-[#1e40af] to-[#3b82f6] text-white py-4 rounded-xl text-center font-bold mt-4 shadow-lg flex items-center justify-center gap-2">
            <Image src="/whatsapp-icon.svg" alt="WhatsApp" width={16} height={16} className="invert" /> Chat WhatsApp Sekarang
          </Link>
        </div>
      )}
    </nav>
